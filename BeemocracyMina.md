...
#### Possible Exploits and Counter Measures
  * Exploit: Loading the Random Jury Pool With Elite Scout wallets Under The Control Of A Special Interest
    ...
    * Counter Measure: ZK-Ceremony and demonstration of human memory for elite scouts that wish to become jurors: 
      ...
      * This message proves the anonymous reputation is controlled by this human without revealing who the human is and without revealing the 24 words that control the wallet.
    ...
    * Exploit: Jurors selling their wallet keys (selling their entire anonymous reputation to special interest):
      ...
      * That leaves some preregistered biometrics (I don't like the idea of collecting biometrics), ability to recall of previous conversations in public forums, match voice in previous public conversations with current voice.
      ...
      * Might be interesting to have a hardware wallet that generates a 25th word from a fingerprint reader.
        * The fingerprint reader would be built into the hardware wallet in such a way that trying to get at the signal wires to read the extra 25th word generated by the fingerprint would destroy the entire device.
        * So hardware wallets would be interchangeable, but impossible to sell without also selling the fingerprint.
        * One could just get the values generated by a different fingerprint reader of the same type but not connected to the hardware wallet, so this idea won't work unless each reader produces unique salt.
      * Sounds like a job for the a.i. controlled hardware wallet [described at this link here](https://github.com/johnshearing/A.i.-Classifiers-Zero-Knowledge-Proofs-Without-Zero-Knowledge-Cryptography).
        * The idea is a hardware wallet with onboard a.i. that is trained to recognize only it's owner.
        * The device would have camera, microphone, fingerprint reader, and keyboard for input and [look something like this hardware wallet from a previous project](https://github.com/johnshearing/PrivateKeyVault/blob/master/README.md).
        ...
        * The device would have a [clonable encrypted sd card for backup](https://github.com/johnshearing/PrivateKeyVault?tab=readme-ov-file#cloning-your-encrypted-sd-card-using-only-the-raspberry-pi) incase the original device is lost or stolen.
